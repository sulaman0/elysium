...
import {
    ...
    ETH_BLOCK_GAS_LIMIT,
    ETH_BLOCK_POV_LIMIT,
    TEST_ERC20_BYTECODE, GAS_PRICE, GAS, GAS_DECIMAL,
} from "./config";
...
describeWithFrontier("Frontier RPC (Gas limit Weightv2 pov size)", (context) => {
    ...
    step("gas limit bound works with pov size heavy txns", async function () {
        ...
        expect(latest.gasUsed).to.be.lessThanOrEqual(ETH_BLOCK_GAS_LIMIT + 1500);
        expect(ETH_BLOCK_GAS_LIMIT - latest.gasUsed).to.be.lessThan(parseInt(GAS_DECIMAL));
    });
});
