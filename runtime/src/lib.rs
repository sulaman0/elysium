...
use sp_runtime::{create_runtime_str, generic, impl_opaque_keys, traits::{
	...
	IdentifyAccount, NumberFor, PostDispatchInfoOf, Verify, OpaqueKeys
}, transaction_validity::{TransactionSource, TransactionValidity, TransactionValidityError}, ApplyExtrinsicResult, MultiSignature, Perbill, Permill};
...
use pallet_evm::{
	...
	HashedAddressMapping, Runner,EVMCurrencyAdapter
};
...
pub struct ElysiumGasWeightMapping;
impl GasWeightMapping for ElysiumGasWeightMapping {
...
impl FeeCalculator for FixedGasPrice {
	fn min_gas_price() -> (U256, Weight) {
		// let min_gas_price = currency::GIGAWEI.saturating_mul(currency::SUPPLY_FACTOR as u128); //this was old calculation formula now updated with new one.
		let min_gas_price = currency::WEIGHT_FEE.saturating_mul(WEIGHT_PER_GAS as u128);
		(
			min_gas_price.into(),
...
parameter_types! {
	...
	pub const ChainId: u64 = CHAIN_ID; // for main net
}

impl pallet_evm::Config for Runtime {
	type FeeCalculator = FixedGasPrice;
	type GasWeightMapping = ElysiumGasWeightMapping;
