...
impl<T: Config> Pallet<T> {
    ...
    fn validate_transaction_in_pool(
        ...
        let (base_fee, _) = T::FeeCalculator::min_gas_price(Option::from(&origin));
        ...
        log::info!(
            "===pallet-ethereum========= INFO Zero gas price applied for sender: {:?}",
            &origin,
        );
    ...
    pub fn validate_transaction_in_block(
        ...
        log::info!(
            "pallet-ethereum validate_transaction_in_block========= INFO Zero gas price applied for sender: {:?}",
            &origin,
        );
