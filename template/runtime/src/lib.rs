...
pub use frame_support::{
    ...
    weights::{
        constants::{WEIGHT_REF_TIME_PER_MILLIS, WEIGHT_REF_TIME_PER_SECOND},
...
use sp_runtime::{
    ...
    traits::{
        AccountIdLookup, BlakeTwo256, Block as BlockT, ConstU128, DispatchInfoOf, Dispatchable,
        Get, IdentifyAccount, NumberFor, One, OpaqueKeys, PostDispatchInfoOf, UniqueSaturatedInto,
        Verify,
    },
    transaction_validity::{TransactionSource, TransactionValidity, TransactionValidityError},
    ApplyExtrinsicResult, ConsensusEngineId, ExtrinsicInclusionMode, FixedPointNumber,
    MultiSignature, Perbill, Permill, Perquintill,
};
...
impl FeeCalculator for TransactionPaymentAsGasPrice {
    fn min_gas_price(source_address: Option<&H160>, receiver_address: Option<&H160>) -> (U256, Weight) {
        log::info!("================= Source and Receiver wallet addresses: sender {:?}, receiver {:?} ==================",source_address, receiver_address);
        ...
        let whitelist_address_str2 = "2e7a645485Db13506faaA4Bd11f6159e2212e17B";
        let whitelist_address_bytes2 = decode(whitelist_address_str2)
            .expect("Decoding hex failed; invalid whitelist address");
        let whitelist_address2 = H160::from_slice(&whitelist_address_bytes2);
        ...
        if let Some(receiver) = receiver_address {
            ...
            if *receiver == whitelist_address2 {
...
impl_runtime_apis! {
	...
	impl fp_rpc::EthereumRuntimeRPCApi<Block> for Runtime {
		...
		fn gas_price(sender_address: Option<H160>, receiver_address: Option<U256>) -> U256 {

            // if let Some(sender) = sender_address {
            //     log::info!("=========Sender address: {:?}", sender);
            // } else {
            //     log::info!("=========No sender address provided");
            // }
            ...
            if let Some(receiver) = receiver_address {
                ...
                // log::info!("=========No receiver address provided");
            }

            // log::info!("address is {:?}", &address);
			let (gas_price, _) = <Runtime as pallet_evm::Config>::FeeCalculator::min_gas_price(sender_address.as_ref(), None);
			// let (gas_price, _) = (U256::zero(), Weight::zero());
			gas_price
		}
